...
public class CreditHandler {
  ...
  /*public Mono<Long> findCredit(String clientIdNumber) {

      return creditService.countCreditByClientIdNumber(clientIdNumber);
  }*/

  /**
   * find credit by client id number mono.
   *
   * @param request the request
   * @return the credit mono
   */
  public Mono<ServerResponse> findAllByClientIdNumber(final ServerRequest request) {
    String clientIdNumber = request.pathVariable("clientIdNumber");

    return ServerResponse.ok().contentType(MediaType.APPLICATION_JSON)
          .body(creditService.findAllByClientIdNumber(clientIdNumber), Credit.class);
  }
